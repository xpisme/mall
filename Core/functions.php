...
function get_crumbs($cid)
{
    ...
    foreach ($parray as $v) {
        if ($v) {
            array_unshift($cname, M('cate')->getRow('cid,cname', 'cid='.$v));
        }
    }
...
{
    foreach ($data as $k=>$v) {
        if (isset($v['thumb_img'])) {
            $data[$k]['thumb_img'] = explode(',', $v['thumb_img']);
        }
        if (isset($v['ori_img'])) {
            $data[$k]['ori_img'] = explode(',', $v['ori_img']);
        }
    }
    return $data;
}
